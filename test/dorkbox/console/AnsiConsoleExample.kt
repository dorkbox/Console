...
object AnsiConsoleExample {
    ...
    fun main(args: Array<String>) {
        ...
        println(
            Ansi.ansi().a("Should have two blank spots in the above 0's")
                .saveCursorPosition()
                .cursorUp(4).cursorLeft(30).a("   ")
                .cursorDownLine().cursorRight(5).a("   ")
                .restoreCursorPosition()
        )


        println("ver : " + AnsiRenderer.render("@|bold,green ${Console.version}|@") )
        println("console type: " + AnsiRenderer.render("@|bold,red ${Console.`in`.javaClass}|@") + " (${Terminal.terminal.width}w x ${Terminal.terminal.height}h)")
        ...
        while (Console.`in`.read().also { read = it } != 'q'.code) {
            if (Character.isDigit(read)) {
                ...
                if (numericValue == 2) {
                    Console.out.print(Ansi.ansi().cursorLeft().scrollDown(1))
                }
                else {
                    Console.out.print(Ansi.ansi().cursorLeft().scrollUp(numericValue))
                }
            } else {
                Console.out.println(" char : " + read + " (" + read.toChar() + ")")
            }

            // not needed because our ANSI stream is auto-flushing now.
//            Console.out.flush() // flush guarantees the terminal moves the way we want
        }
